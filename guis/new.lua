...
local getcustomassets = {
	...
	['vain/assets/mascot.png'] = 'rbxassetid://110703296275845',
	...
	['vain/assets/friendstab.png'] = 'rbxassetid://106739759659697',
...
-- Vain reads assets from local files only; no remote downloads.
local function downloadFile(path, func)
	return (func or readfile)(path)
end

-- Always resolve assets from the built-in rbxassetid lookup table.
...
mainapi.Libraries = {
	...
	tween = tween,
	uipallet = uipallet,
	base64 = (isfile('vain/libraries/base64.lua') and loadstring(readfile('vain/libraries/base64.lua'), 'base64')()) or {},
}
...
function mainapi:CreateGUI()
	...
	discordbutton.Image = getcustomasset('vain/assets/discord.png')
	...
	settingsversion.Text = 'Vain v'..mainapi.Version
	...
	-- discordbutton is kept in the UI layout but does nothing in Vain.
...
function mainapi:CreateCategoryList(categorysettings)
	...
	settings.Image = categorysettings.Profiles and getcustomasset('vain/assets/worldicon.png') or getcustomasset('vain/assets/customsettings.png')
...
function mainapi:CreateProfileGUI()
	...
	icon.Position = UDim2.fromOffset(10, 13)
	icon.BackgroundTransparency = 1
	icon.Image = getcustomasset('vain/assets/profilesicon.png')
	icon.ImageColor3 = uipallet.Text
	icon.Parent = window
	...
	TextButton.MouseButton1Click:Connect(function()
		...
		if lol then
			...
			writefile('vain/profiles/'..awesome..mainapi.Place..'.txt', file)
			mainapi:Load(true, awesome)
			mainapi:CreateNotification('Vain', `Downloaded "{info.Text}" by {lol.discord_username}`, 5, 'info')
		else
			mainapi:CreateNotification('Vain', `Failed to fetch config ({info.Text})`, 10, 'warning')
		end
	end)

	publishb.MouseButton1Click:Connect(function()
		configapi.ShowPopup(false)
		-- Public config publishing is not available in Vain.
		mainapi:CreateNotification('Vain', 'Public profiles are not available in this build.', 5, 'info')
	end)

	ImageLabel.MouseButton1Click:Connect(function()
		-- Public config deletion is not available in Vain.
		mainapi:CreateNotification('Vain', 'Public profiles are not available in this build.', 5, 'info')
	end)
	...
	profilemaker.MouseButton1Click:Connect(function()
		-- Public profiles are not available in Vain. No remote API is contacted.
		mainapi:CreateNotification('Vain', 'Public profiles are not available in this build.', 5, 'info')
	end)
...
function mainapi:Load(skipgui, profile, profiledata)
	...
	if isfile('vain/profiles/'..self.Profile..self.Place..'.txt') then
		local savedata = profiledata or loadJson('vain/profiles/'..self.Profile..self.Place..'.txt')
	...
	if shared.VainDeveloper or (inputService.TouchEnabled or not inputService.KeyboardEnabled) and #self.Keybind == 1 and self.Keybind[1] == 'RightShift' then
		...
		image.Image = getcustomasset('vain/assets/mascot.png')
		image.Parent = button
...
mainapi:CreateCategory({
	Name = 'Legit',
	Icon = getcustomasset('vain/assets/legittab.png'),
	Size = UDim2.fromOffset(14, 14)
})
...
guipane:CreateDropdown({
	...
	Tooltip = 'new - The default Vain theme\nold - The classic Vain theme\nrise - Rise 6.0'
})
